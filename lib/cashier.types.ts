...
export interface CashierRow {
  ...
  dept: string;
  /** เวลาที่ถูกส่งมาห้องการเงิน HH:MM (ไว้แสดงผลเท่านั้น) */
  time: string;
  /**
   * เวลาที่กด "เรียกคิว" แบบเต็ม (ถึงวินาที) — มีเฉพาะแถวจาก sd_queue_calling
   * ใช้ทำ key กันเรียกซ้ำเท่านั้น ห้ามเอาไปแสดงผลแทน `time`
   * (`time` ปัดเหลือนาที เรียกคนเดิมซ้ำในนาทีเดียวกันจะได้ค่าเดียวกัน
   *  ทำให้จอเข้าใจผิดว่าเป็นการเรียกครั้งเดิมแล้วไม่ประกาศเสียงซ้ำให้)
   */
  callSeq?: string;
