<?php
    ...
    $query = "SELECT o.main_dep_queue,o.oqueue,o.hn,CONCAT(pt.pname,pt.fname,' ','XXX') AS full_name,CONCAT('คุณ',pt.fname,' ',pt.lname) AS full_namecall,IFNULL(o.pt_priority,0) as 	pt_priority,
    ...
    WHERE /*o.ovstost = '00' AND*/ sq.sd_queue_calling_curdep IN(".$depcode.")  AND DATE(sq.sd_queue_calling_datetime) = CURDATE() 
    ...
    while($row =mysql_fetch_assoc($result))
    {
        if ($row["sd_queue_calling_status"] == "Y"){
            $txt .= " ขอเชิญ ".$row["full_namecall"]." ที่จุด ".$row["department"]." ค่ะ ";
